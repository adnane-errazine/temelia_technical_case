...
def format_chunk_html(chunk: Dict[str, Any], index: int) -> str:
    """Enhanced chunk formatting with interactive elements"""
    ...
    short_text = chunk_text[:max_display_length]
    is_collapsed = len(chunk_text) > max_display_length
    
    # Prepare the conditional HTML sections separately
    collapsed_text_div = f"<div class='text-full' style='display:none;'>{chunk_text}</div>" if is_collapsed else ""
    
    chunk_actions_div = ""
    if is_collapsed:
        chunk_actions_div = """<div class='chunk-actions'>
            <button onclick='toggleText({0})' class='text-toggle'>
                <span class='expand'>▼ Voir plus</span>
                <span class='collapse' style='display:none'>▲ Voir moins</span>
            </button>
        </div>""".format(index)
    
    html = f"""
    <div class="chunk-card" id="chunk-{index}">
        ...
        <div class="chunk-content">
            <div class="text-preview">{short_text}</div>
            {collapsed_text_div}
        </div>
        {chunk_actions_div}
    </div>
...
function toggleText(index) {
    const card = document.querySelector(`#chunk-${index}`);
...
def main():
    ...
    with gr.Blocks(
        ...
        chunks_state.change(
            lambda x: "".join(json.loads(x)) if x else "<p style='color: #666; text-align: center;'>Aucune source utilisée</p>",
            inputs=[chunks_state],
            outputs=[chunks_html]
        )
        ingest_button.click(
