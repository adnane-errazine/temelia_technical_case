...
from src.parsing.parser_mistral import parse_document as mistral_parse
...
def process_pdf(
    file_path: str, username: str, dense_embedder, sparse_embedder, gpu: bool = True
) -> Generator:
    """
    ...
    try:
        ...
        if gpu:
            ...
            parsed_df = mistral_parse(file_path)
        else:
        ...
        yield "Chunking the text with appropriate sizing..."
        ...
        yield "Generating dense embeddings..."
        ...
        yield "Generating sparse embeddings..."
        ...
        yield "Storing document embeddings in vector database..."
        ...
        print(
            f"  - PDF parsing: {execution_times['parsing']:.2f}s ({(execution_times['parsing'] / total_time * 100):.1f}%)"
        )
        print(
            f"  - Text chunking: {execution_times['chunking']:.2f}s ({(execution_times['chunking'] / total_time * 100):.1f}%)"
        )
        print(
            f"  - Dense embeddings: {execution_times['dense_embeddings']:.2f}s ({(execution_times['dense_embeddings'] / total_time * 100):.1f}%)"
        )
        print(
            f"  - Sparse embeddings: {execution_times['sparse_embeddings']:.2f}s ({(execution_times['sparse_embeddings'] / total_time * 100):.1f}%)"
        )
        print(
            f"  - Database operations: {execution_times['vectorstore_operations']:.2f}s ({(execution_times['vectorstore_operations'] / total_time * 100):.1f}%)"
        )
        print(
            f"  - CSV export: {execution_times['csv_export']:.2f}s ({(execution_times['csv_export'] / total_time * 100):.1f}%)"
        )
