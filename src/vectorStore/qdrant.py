...
def get_qdrant_client() -> QdrantClient:
    ...
    if qdrant_client is None:
        raise ConnectionError(
            "Qdrant client is not initialized. Check configuration and connection."
        )
...
def create_collection(collection_name: str, recreate: bool = False) -> None:
    ...
    try:
        client = (
            get_qdrant_client()
        )  # Get client, raises ConnectionError if not initialized
        ...
        print(
            f"ERROR: Could not connect to Qdrant for collection operations: {ce}",
            file=sys.stderr,
        )
        raise  # Re-raise connection error
    except Exception as e:
        print(
            f"ERROR: Failed during collection creation/check for '{collection_name}': {e}",
            file=sys.stderr,
        )
        raise  # Re-raise other exceptions


def upsert_embeddings_from_df(
    collection: str, df_input: pd.DataFrame, batch_size: int = 100
) -> None:
    """
    ...
    """
    try:
        qdrant_client_local = get_qdrant_client()
        print(
            f"Starting upsert process for collection '{collection}' with {len(df_input)} rows."
        )
        batch = []
        for _, row in df_input.iterrows():
            ...
            sparse_vec = models.SparseVector(
                indices=list(row["sparse_indices"]), values=list(row["sparse_values"])
            )
            # Build point
