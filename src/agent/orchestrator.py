...
class Orchestrator:
    ...
    def rerank_chunks(self, query: str, hits: List[Dict[str, Any]], top_k: int = 10) -> List[Dict[str, Any]]:
        """
    ...
    def ask_pipeline(self, user_question: str,qdrant_collection:str, dense_limit:int=30, sparse_limit:int=30, top_k_reranker:int=10,filters=None) -> Tuple[str, list]:
        # 1. Enhance the query
        enhanced_q = enhance_query(user_question)

        # 2. Retrieve chunks
        hits = self.retriever.retrieve(text=enhanced_q,qdrant_collection=qdrant_collection , dense_limit=dense_limit,sparse_limit=sparse_limit)
        
        
        # BONUS: reranking relevant chunks through a cross-encoder model (COHERE for this POC)
        reranked_hits = self.rerank_chunks(enhanced_q, hits, top_k=top_k_reranker)
        
        # 2.1. Extract texts and metadata from hits
        chunks_payload_with_id = [{**hit['payload'], 'chunk_id': hit['id'], 'score':hit['score']} for hit in reranked_hits]
            
        # 3. Generate answer
        chat_hist = get_chat_history()
        
        
        response_gen_rag = main_rag_llm_answerer(enhanced_q, chunks_payload_with_id, chat_hist)
...
if __name__ == "__main__":
    ...
    user_question_test = "Quelles sont les exigences minimum requises pour les dispositifs d'isolation?"
    ...
    for partial_answer, chunks_metadata in orchestrator.ask_pipeline(user_question_test):
